import React, { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Pen, Plus, Search, Trash } from "lucide-react";
import { useExamStore } from "@/stores/useExamStore";
import CreateExam from "./CreateExam";

const ExamManager = () => {
  const { exams, getAllExam, createExam, deleteExam } = useExamStore();
  const [isOpenCreateExam, setIsOpenCreateExam] = useState(false);

  useEffect(() => {
    const fetchExam = async () => {
      await getAllExam();
    };

    fetchExam();
  });

  return (
    <div className="w-full h-full p-6 bg-gray-50 rounded-lg shadow">
      {/* Header */}
      <div className="flex items-center justify-between border-b pb-4">
        <h1 className="text-2xl font-semibold text-gray-800">Quản lý kỳ thi</h1>
        <div className="flex items-center gap-3">
          {/* Search box */}
          <div className="relative">
            <Input
              className="pe-12 rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500"
              type="text"
              placeholder="Nhập kỳ thi cần tìm"
            />
            <Button
              className="absolute right-0 top-0 h-full px-3 text-gray-500 hover:text-blue-600"
              variant="ghost"
            >
              <Search />
            </Button>
          </div>

          <Button
            onClick={() => setIsOpenCreateExam(true)}
            variant="blue"
            className="flex items-center gap-2 px-4 py-2 rounded-md shadow hover:opacity-90 cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            Thêm kỳ thi
          </Button>

          <CreateExam
            isOpen={isOpenCreateExam}
            isClose={() => setIsOpenCreateExam(false)}
            func={createExam}
          />
        </div>
      </div>

      {/* Table */}
      <div className="mt-6 overflow-x-auto">
        <table className="w-full border-collapse bg-white rounded-lg shadow">
          <thead className="bg-gray-100 text-gray-700">
            <tr className="uppercase text-sm">
              <th className="px-4 py-2">Tên kỳ thi</th>
              <th className="px-4 py-2">Số câu hỏi</th>
              <th className="px-4 py-2">Thời gian (phút)</th>
              <th className="px-4 py-2">Ngày tạo</th>
              <th className="px-4 py-2">Chức năng</th>
            </tr>
          </thead>
          <tbody>
            {exams.map((e) => (
              <tr
                key={e._id}
                className="border-t hover:bg-gray-50 transition-colors"
              >
                <td className="px-4 py-2">{e.title}</td>
                <td className="px-4 py-2 text-center">{e.questions.length}</td>
                <td className="px-4 py-2 text-center">
                  {Math.floor(e.timeLimit / 60)}
                </td>
                <td className="px-4 py-2 text-center">
                  {new Date(e.createdAt).toLocaleDateString("vi-VN")}
                </td>
                <td className="px-4 py-2 flex justify-center items-center gap-2">
                  <Button
                    size="sm"
                    variant="blue"
                    className="cursor-pointer rounded-md shadow"
                  >
                    <Pen className="w-4 h-4" />
                  </Button>
                  <Button
                    onClick={async () => deleteExam(e._id)}
                    variant="ghost"
                    size="sm"
                    className="bg-red-500 text-white hover:bg-red-600 rounded-md shadow cursor-pointer"
                  >
                    <Trash className="w-4 h-4" />
                  </Button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default ExamManager;
